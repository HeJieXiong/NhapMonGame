...
Key functions
CGameSweptAABB
CGameObjectSweptAABBEx
CGameObjectCalcPotentialCollisions
CGameObjectFilterCollision

CGameObjectGetBoundingBox
...
#include Stage5.h
#include Simon.h
#define WINDOW_CLASS_NAME LSampleWindow
#define MAIN_WINDOW_TITLE L04 - Collision
...
CStage4		stage4;
CStage5		stage5;
CMorningstarmorningstar;


CHeaderBar	headerbar;

CKnife		knife;
vectorLPGAMEOBJECT objects_weapons;
int i;
int current_stage=1;
...
void CSampleKeyHanderOnKeyDown(int KeyCode)
{
	DebugOut(L[INFO] KeyDown %dn, KeyCode);
	switch (KeyCode)
	{
	case DIK_X
...
void CSampleKeyHanderOnKeyUp(int KeyCode)
{
	DebugOut(L[INFO] KeyUp %dn, KeyCode);
	switch (KeyCode)
	{
	case DIK_UP
...
void CSampleKeyHanderKeyState(BYTE states)
{
